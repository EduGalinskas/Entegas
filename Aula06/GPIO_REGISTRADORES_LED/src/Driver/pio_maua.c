...
void _pio_set_output(	Pio *p_pio, 
                const uint32_t ul_mask,
		       	...
		       	const uint32_t ul_pull_up_enable){
					   p_pio->PIO_PER = (ul_mask & ul_pull_up_enable); //Habilita MUX do buffer de saída
					   p_pio->PIO_OER = (ul_mask & ul_pull_up_enable); //Habilita Buffer de saída
...
void _pio_pull_up(	Pio *p_pio, 
               const uint32_t ul_mask,
		            const uint32_t ul_pull_up_enable){
					p_pio->PIO_PUER = (ul_mask & ul_pull_up_enable);	
					}
